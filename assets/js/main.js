/*
 * START: AHH MAA - ELEMENTOR LOOP GRID AJAX FILTER v3.3
 */
jQuery(function ($) {
	...
	$(window).on("load", function () {
		...
		function initializeFilters() {
			...
			filterInstances.each(function () {
				...
				const loopGrid = gridInstances.filter('[data-filter-type="' + filterType + '"]');

				if (loopGrid.length) {
					// Create a new ProductFilter instance for each filter/grid pair
		...
		function ProductFilter(filterContainerEl, loopGridEl) {
			...
			this.setupLoadMore = function () {
				if (self.loopGrid.next(".load-more-container").length === 0) {
					self.loopGrid
						.not(".ahh-maa-desktop-carousel")
						.after('<div class="load-more-container"><a href="#" class="load-more-button elementor-button">Load More</a></div>');
			...
			this.updateLoadMoreButton = function () {
				const loadMoreButton = self.loopGrid.next(".load-more-container").find(".load-more-button:visible");
			...
			this.initializeCarousel = function () {
				...
				// Use imagesLoaded to ensure carousel calculates height correctly
				self.loopContainer.imagesLoaded(function () {
					const groupCellsValue = window.innerWidth < 1025 ? 2 : 3;
				...
				$.ajax({
					...
					success: function (response) {
						if (response.success) {
							...
							// *** THE FIX IS HERE ***
							// After loading new products via AJAX, re-run the styling function.
							// This ensures new items get the 'in-cart' class if needed.
							updateInCartStyles();
...
/*
 * END: AHH MAA - ELEMENTOR LOOP GRID AJAX FILTER v3.3
 */
